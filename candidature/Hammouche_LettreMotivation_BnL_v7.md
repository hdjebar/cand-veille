...
Intelligent Luxembourg Heritage part de là. Sur les données de référence de la BnL, le prototype atteint une précision quatre fois supérieure au système OCR actuellement en production — avec un modèle VLM (Vision Language Model : un modèle d’IA qui lit simultanément l’image et le texte des scans), sans infrastructure dédiée, sans phase d’entraînement. Les noms de personnes, de lieux et d’organisations extraits des journaux anciens sont localisés avec précision dans les scans, ce qui permettra de les surligner directement à l’écran. Entièrement basé sur les données ouvertes CC0 de data.bnl.lu, conçu pour s’intégrer dans l’infrastructure existante, gouvernance formalisée et EU AI Act dès la conception.

Ce travail s’inscrit dans un contexte européen : le benchmark international de référence en traitement des documents historiques multilingues inclut cette année le luxembourgeois pour la première fois. La soumission est prête. La BnF enrichit Gallica. La Bibliothèque nationale des Pays-Bas a déployé son outil. Le corpus luxembourgeois manque dans les deux projets. Le nôtre existe, en accès libre. Il sera mis à disposition pour les chercheurs. — Fichier PropILH.html joint : documentation complète, s’ouvre en double-cliquant.

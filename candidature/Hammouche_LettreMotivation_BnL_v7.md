...
Intelligent Luxembourg Heritage part de là. Le POC tourne sur les données CC0 de data.bnl.lu — les mêmes images de référence que la baseline officielle Nautilus : cMER 1,38 % zero-shot avec un modèle open weights (Qwen3.5-397B-A17B), contre 3,68 % pour Nautilus/Kraken fine-tuné (Schneider 2023). Le pipeline est conçu pour une connexion directe aux systèmes productifs BnL : METS/ALTO → VLM → NER → entity linking Wikidata/ARK, piloté sous QUAPITAL-HERMES avec EU AI Act en Phase 0.

HIPE-OCRepair 2026 (ICDAR) organise le premier benchmark international de correction post-OCR par LLM incluant le luxembourgeois, via Impresso. La soumission est prête pour avril — gain mesuré de +2,30 pp zero-shot. La BnF a Gallica/ArGiMi. La KB Netherlands a Retrotool. Le corpus luxembourgeois manque dans les deux projets. Le nôtre est en CC0. Il sera mis à disposition pour accompagner les chercheurs dans l’usage des données ouvertes — au service du secteur bibliothèques, archives et musées. — Fichier PropILH.html joint, s’ouvre en double-cliquant, aucune installation requise.

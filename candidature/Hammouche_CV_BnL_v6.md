...
Le programme de travail BnL 2025-2028 pose un défi précis : 171 projets à piloter, des initiatives IA à transformer en production pérenne, des collections à valoriser pour les chercheurs qui en ont besoin maintenant. C'est exactement ce type de travail que je construis depuis dix ans dans des institutions à gouvernance formelle. Intelligent Luxembourg Heritage, développé sur les données CC0 de data.bnl.lu, en donne la mesure : CER 2,72 % contre la baseline Nautilus/Kraken publiée, sans fine-tuning, piloté sous QUAPITAL-HERMES composé avec CRISP-ML(Q). Je veux apporter ce travail à la BnL et le développer avec les équipes qui connaissent les collections et les chercheurs qui en dépendent.
...
**2014 – présent** *(missions en parallèle)*

**Intelligent Luxembourg Heritage (2026 – en cours)**  
POC zero-shot mesuré sur les données officielles BnL (CC0) : CER 2,72 % vs Nautilus/Kraken 3,68 % publié, sans fine-tuning, sur les mêmes images de référence. Pipeline : images METS/ALTO → VLM (Qwen3.5-397B-A17B) → post-correction OCR + NER (PER, LOC, ORG, DATE) + bbox_2d en une passe → entity linking Wikidata/ARK → knowledge graph bibnet.lu. Périmètre Phase 1-2 : 800 000 pages, presse historique DE/FR/LB, XIXe–XXe siècle. Phase 3+ : pipeline vidéo OT→IT — robot feuilleteur (convoyeur RFID + caméra) filme les documents des magasins en continu, extraction de frames, traitement VLM avec contexte temporel. Déploiement souverain sur MeluXina-AI/CTIE. Piloté sous QUAPITAL-HERMES composé avec CRISP-ML(Q) : trois portes de décision, registre des risques, quality gates. Conformité EU AI Act intégrée dès la conception : Art. 50, model card, AIPD données historiques. Partenariats engagés : Uni.lu, C²DH, Impresso Phase 2.  
→ [github.com/hdjebar/IntelligentLuxembourgHeritage](https://github.com/hdjebar/IntelligentLuxembourgHeritage)

**Plateforme RAG multimodale (2024–2025)**  
Conduit, avec les équipes métier et techniques, la conception et le déploiement d'une plateforme de recherche sémantique sur des dépôts hétérogènes (documents, images, données structurées). Pipeline : ingestion multimodale → post-correction OCR → segmentation → embeddings → chaîne LangChain avec attribution des sources → enrichissement NER pour la recherche structurée. Ce qui prenait des heures de navigation manuelle entre silos se résout maintenant en quelques secondes. *POC fonctionnel disponible pour démonstration.*

**Plateforme d'aide à la décision architecturale**  
...
**Système d'analyse prédictive en santé**  
Développé, aux côtés des équipes médicales de l'Agence de la Biomédecine (France), des algorithmes de scoring ML pour l'allocation de greffons hépatiques. De la préparation des données à la validation clinique et à l'intégration dans les protocoles : un cycle complet, avec les cliniciens à chaque étape.

**Plateforme intelligence emploi**  
Automatisé, avec les équipes des services de l'emploi, la mise en relation profil/formation par analyse NLP des compétences. Un processus manuel, sous-doté en ressources, qui touchait désormais beaucoup plus de bénéficiaires.

**Système de transcription d'entreprise**  
Pipeline NLP de transcription automatique de réunions (français, diarisation, résumé) : comptes-rendus le jour même, là où le cycle manuel prenait une semaine.
...
- Construit, avec les parties prenantes réglementaires, une stratégie cloud conforme aux exigences CSSF : gouvernance, vocabulaire réglementaire, alignement multi-entités.
...
Co-construit avec la direction la pratique EA interne. Automatisé les opérations de données ServiceNow via PowerShell. Co-défini, avec les équipes Business Architecture et IT, les frameworks de gouvernance Microsoft Power Platform. Architecturé les capacités d'analytics cloud pour une BI quasi-temps réel.
...
Audits IT, transformation enterprise, optimisation processus santé (approche TLS (Theory of Constraints, Lean, Six Sigma)). Missions incluant l'analyse et la restructuration de flux physiques : logistique de distribution, gestion de magasinage, automatisation d'entrepôt. Co-fondé LuxBA et LuxEA, les deux communautés de pratique luxembourgeoises en analyse métier et architecture d'entreprise.
...
Conçu et mis en oeuvre, avec les équipes RH, les systèmes de gestion du personnel sur la suite Oracle ERP. Co-construit avec la direction le PMO IT du Parlement : standards de gouvernance, méthodologies, livraison prévisible.

...
Le programme de travail BnL 2025-2028 pose un défi précis : 171 projets à piloter, des initiatives IA à transformer en production pérenne, des collections à valoriser pour les chercheurs qui en ont besoin maintenant. C’est exactement ce type de transition — du prototype qui prouve à la solution qui tient dans la durée — que je construis depuis dix ans dans des institutions à gouvernance formelle. Intelligent Luxembourg Heritage, développé sur les données CC0 de data.bnl.lu, en donne la mesure : CER 2,72 % contre la baseline Nautilus/Kraken publiée, sans fine-tuning, piloté sous QUAPITAL-HERMES composé avec CRISP-ML(Q). Je veux apporter ce travail à la BnL et le développer avec les équipes qui connaissent les collections et les chercheurs qui en dépendent.
...
POC zero-shot mesuré sur les données officielles BnL (CC0) : CER 2,72 % vs Nautilus/Kraken 3,68 % publié, sans fine-tuning, sur les mêmes images de référence. Périmètre : post-correction OCR multimodale (800 000 pages, presse historique DE/FR/LB), NER/entity linking multilingue, déploiement souverain sur MeluXina-AI. Piloté sous QUAPITAL-HERMES, adapté et composé avec CRISP-ML(Q) — trois portes de décision, registre des risques, quality gates. Conformité EU AI Act intégrée dès la conception : Art. 50 pour le chatbot, model card et AIPD pour le pipeline ILH. Partenariats engagés : Uni.lu, C²DH, Impresso Phase 2.  
...
Développé, aux côtés des équipes médicales de l’Agence de la Biomédecine (France), des algorithmes de scoring ML pour l’allocation de greffons hépatiques. De la préparation des données à la validation clinique et à l’intégration dans les protocoles décisionnels : un cycle complet, avec les cliniciens à chaque étape.
...
Audits IT, transformation enterprise, optimisation processus santé (approche TLS). Co-fondé LuxBA et LuxEA, les deux communautés de pratique luxembourgeoises en analyse métier et architecture d’entreprise.
